-- Actualiza la vista productos_with_cost para considerar ingredientes por categoría
    ...
    CASE
      ...
      WHEN ri.use_categorias THEN EXISTS (
        ...
        WHERE iws_cat.user_id = p.user_id
          ...
          AND COALESCE(to_jsonb(iws_cat.categoria_ids), '[]'::jsonb) @> COALESCE(ri.required_categoria_ids, '[]'::jsonb)
